...
@userge.on_cmd(
    "lastfm", about={"header": "Shows currently scrobbling track or most recent scrobbles if nothing is playing."}
)
async def last_fm_pic_(message: Message):
    user = await userge.get_me()
    base_url = "http://ws.audioscrobbler.com/2.0"
    res = await AioHttp.get_json(f"{base_url}?method=user.getrecenttracks&limit=3&extended=1&user={Config.LASTFM_USERNAME}&api_key={Config.LASTFM_API_KEY}&format=json")
    if not res[0] == 200:
        return await message.edit("Hmm... something went wrong.\nPlease ensure that you've set the correct username!")
    res = res[1]
    try:
    ...
    if first_track.get("@attr"):
        ...
        last_user = (await AioHttp.get_json(f"{base_url}?method=user.getinfo&user={Config.LASTFM_USERNAME}&api_key={Config.LASTFM_API_KEY}&format=json"))[1].get("user")
