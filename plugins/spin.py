...
async def spinn(message: Message):
    ...
    await runcmd(f"ffmpeg -framerate {frate} -i {path}spinx%d.jpg -c:v libx264 -pix_fmt yuv420p {output_vid}")
    if os.path.exists(output_vid):
        await message.delete()
        if '-r' in message.flags:
            round_vid = os.path.join(path, "out_round.mp4")
            await crop_vid(output_vid, round_vid)
            await message.client.send_video_note(message.chat.id, round_vid, reply_to_message_id=reply_id)
        else:
            await message.client.send_animation(message.chat.id, output_vid, unsave=(not message.client.is_bot), reply_to_message_id=reply_id)
    os.remove(pic_loc)
    shutil.rmtree(path)
